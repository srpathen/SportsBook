...
engine = BrowserEngine("https://sportsbook.fanduel.com/navigation/nfl?tab=receiving-props", headless=False, delay=1)
players = []
...
clickables = engine.getObject(Page.get_by_role, args=["button"]).filter(has_text='Regular Season Total Receiving Yards')
...
for name in names:
    if "2023-24 Regular Season Total Receiving Yards" in name.text:
...
for i in range(len(newNames)):
    players.append([newNames[i].text[:-45], dates[i].text, over[i][2:], overOdds[i], under[i][2:], underOdds[i]])

with open('NFL/ReceivingOdds/Data/FanDuel.csv', 'w', newline='') as file:
